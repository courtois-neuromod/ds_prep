    ...
    for session in sessions_list:
        ...
        for idx, scan in scans.iterrows():
            if '_bold.nii.gz' in scan.filename:
                ...
                acq_files = [acqk_wtiming for acqk_wtiming in acqk_files_startends \
                ...
                if len(acq_files) == 0:
                    ...
                    if len(acq_files) > 1:
                        if not all([acq[1]==acq_files[0][1] for acq in acq_files[1:]]): #duplicated files
                            logging.warning(f"More that one acq file found for: {scan.filename} \n {acq_files}")
                    bname = os.path.basename(acq_files[0][0])
                    dest_path = session_sourcedata / bname
                    matches.append((session/scan.filename, dest_path.relative_to(bids_path)))
