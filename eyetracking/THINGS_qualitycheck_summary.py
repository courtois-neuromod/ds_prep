...
def make_composite_figure(good_runs, y_vals, x_vals, y_vals_cap, out_name=None, y_range=[-0.05, 1.05], driftcorrect=False):
...
def make_gaze_figures(good_runs, calib_gaze_allruns, run_gaze_allruns, out_name=None):
    ...
    for i in range(len(good_runs)):
        cgaze_x, cgaze_y, c_time, cg_xcap, cg_ycap = calib_gaze_allruns[i]
        c_times.append(c_time)
        c_x.append(cgaze_x)
        c_y.append(cgaze_y)
        c_xcap.append(cg_xcap)
        c_ycap.append(cg_ycap)

        rgaze_x, rgaze_y, r_time, rg_xcap, rg_ycap = run_gaze_allruns[i]
        r_times.append(r_time)
        r_x.append(rgaze_x)
        r_y.append(rgaze_y)
        r_xcap.append(rg_xcap)
        r_ycap.append(rg_ycap)

    # calib sequence, X and Y, without drift correction
    make_composite_figure(good_runs, c_x, c_times, c_xcap, out_name=out_name + '_Xposition_calib.png', driftcorrect=False)
    make_composite_figure(good_runs, c_y, c_times, c_ycap, out_name=out_name + '_Yposition_calib.png', driftcorrect=False)
    # main run, X and Y, with and without drift correction
    make_composite_figure(good_runs, r_x, r_times, r_xcap, out_name=out_name + '_Xposition_run.png', driftcorrect=False)
    make_composite_figure(good_runs, r_y, r_times, r_ycap, out_name=out_name + '_Yposition_run.png', driftcorrect=False)
    make_composite_figure(good_runs, r_x, r_times, r_xcap, out_name=out_name + '_Xposition_run.png', driftcorrect=True)
    make_composite_figure(good_runs, r_y, r_times, r_ycap, out_name=out_name + '_Yposition_run.png', driftcorrect=True)


# TODO: simplify function and change name, just need to export line and intercept for QC
def get_linedata(y_val, x_val, mid_val=0.0):
...
def qc_report_summary(list_data, output_path, output_name, data_type, cf_thresh=0.6, tg_thresh = 0.004016):
    ...
    Input:
        ...
        output_path (string) and output_name (string): path and name of output file
        data_type (string): pupil or gaze
    ...
    for i in range(len(list_data)):
        ...
        if cf > cf_thresh:
            if data_type == 'pupils':
                ...
                # Cap values between 0 and 1 to derive polynomials
                x_cap, y_cap, is_off = cap_xy(x, y)
                off_bounds += is_off

                positions.append([x, y, tstamp, x_cap, y_cap])
    ...
    times = pos_array[:, 2].tolist()

    if data_type == 'gaze':
        x_cap = pos_array[:, 3].tolist()
        y_cap = pos_array[:, 4].tolist()
        xdiff, x_m, x_b = get_linedata(x_cap, times, mid_val=0.5)#, y_range=[-0.05, 1.05])
        ydiff, y_m, y_b = get_linedata(y_cap, times, mid_val=0.5)#, y_range=[-0.05, 1.05])

        return (x, y, times, x_cap, y_cap), d1, (xdiff, ydiff, x_m, x_b, y_m, y_b)

    elif data_type == 'pupils':
        xdiff, x_m, x_b = get_linedata(x, times, mid_val=320)#, y_range=[-4.0, 644.0])
        ydiff, y_m, y_b = get_linedata(y, times, mid_val=240)#, y_range=[-4.0, 484.0])
...
def process_run(cfg, run):
    ...
    (cg_x, cg_y, cg_times, cg_xcap, cg_ycap), cg_on2d_d, (xdiff, ydiff, x_m, x_b, y_m, y_b) = qc_report_summary(calib_online_gaze, cfg['out_dir'] + '/qc', 'gaze_calib_online2D_run' + run, 'gaze', cfg['gaze_confidence_threshold'])
    ...
    (rg_x, rg_y, rg_times, rg_xcap, rg_ycap), rg_on2d_d, (xdiff, ydiff, x_m, x_b, y_m, y_b) = qc_report_summary(run_online_gaze, cfg['out_dir'] + '/qc', 'gaze_run_online2D_run' + run, 'gaze', cfg['gaze_confidence_threshold'])
    ...
    gaze_data = ((cg_x, cg_y, cg_times, cg_xcap, cg_ycap), (rg_x, rg_y, rg_times, rg_xcap, rg_ycap))
