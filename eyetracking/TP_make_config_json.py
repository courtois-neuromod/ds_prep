        ...
        if ordered_file_type[idx] == 'CALIB PUPILS':
            if ordered_file_type[idx+1] == 'CALIB PARAM':
                if ordered_file_type[idx+2] == 'RUN PUPILS':
                    ...
                    rcount += 1# Temporary!
                    run_num = str(rcount)# Temporary quick fix for pilot session
...
def update_json(template_json, data_path):
    ...
    list_run_pupils = glob.glob(data_path + '/*pupil/task-triplets*/*/pupil.pldata')
...
if __name__ == "__main__":
    '''
    Script generates config file for a subject's tripplet session

    The config file is compatible with the QC script (quality_check_THINGS_summary.py, which also works for triplets task)
    and can be used for offline calibration (script needs to be adapted from THINGS script)
    '''
    # Load template file

    template_path = os.path.join(args.code_dir, 'config', 'config_TP', 'config_template.json')
    ...
    out_path = os.path.join(args.code_dir, 'config', 'config_TP', 'config_TP_s' + sub_num[-2:] + '_ses' + ses_num[-3:] + '.json')
