...
import os, glob
import retro
retro.data.Integrations.add_custom_path('/home/labopb/Documents/Marie/neuromod/Mario/mario.stimuli')
...
def get_eyetrack_function(gaze_file, time_0 = 0, d3=False, conf_thresh=0.98):
    '''
...
def create_timingdict(dir_path):
    ...
    for ev_path in event_list:
        ...
        timing_data['run_' + run_num] = {}
        timing_data['run_' + run_num]['bk2_dict'] = {}
        ...
        for i in range(ev_file.shape[0]):
            if ev_file['trial_type'][i] == 'gym-retro_game':
                ...
                timing_data['run_' + run_num]['bk2_dict'][fname] = {}
                timing_data['run_' + run_num]['bk2_dict'][fname]['start_event'] = tstamp

    print(timing_data)
    return timing_data


def export_gazemovies(timing_dict, gaze_path, out_path, conf_threshold):
    ...
    for run in timing_dict.keys():
        if run != 'dir_path':
            rnum = run[-1]
            run_dict = timing_dict[run]['bk2_dict']

            run_gpath = gaze_path + '/run' + rnum + '_online_gaze2D.npz'
            ...
            for bk2 in run_dict.keys():
                ...
                try:
                    ...
                    f_xcoord, f_ycoord = get_eyetrack_function(run_gpath, time_0 = onset_time, conf_thresh=conf_threshold)
                    clip = VideoFileClip(video_file)

                    clip_gaze = clip.fx(drawgaze, f_xcoord, f_ycoord, 5)
                    clip_gaze.write_videofile(video_file.split('.')[0]+ '_wgaze.mp4')
...
if __name__ == '__main__':
    ...
    export_gazemovies(timing_dict, gaze_path, out_path, conf_threshold)
